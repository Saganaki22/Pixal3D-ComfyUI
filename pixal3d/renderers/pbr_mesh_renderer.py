...
class EnvMap:
    ...
    def shade(self, gb_pos, gb_normal, kd, ks, view_pos, specular=True):
        try:
            ...
            light_dir = torch.tensor([0.4, 0.7, 0.6], dtype=gb_normal.dtype, device=gb_normal.device)
            light_dir = F.normalize(light_dir, dim=0).view(1, 1, 1, 3)
            diffuse = torch.clamp((gb_normal * light_dir).sum(dim=-1, keepdim=True), min=0.18)
            return kd * diffuse

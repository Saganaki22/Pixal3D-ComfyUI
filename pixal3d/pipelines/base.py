    ...
    def from_pretrained(cls, path: str, config_file: str = "pipeline.json") -> "Pipeline":
        ...
        for k, v in args['models'].items():
            if hasattr(cls, 'model_names_to_load') and k not in cls.model_names_to_load:
                continue
            local_model = f"{path}/{v}"
            if os.path.exists(f"{local_model}.json") and os.path.exists(f"{local_model}.safetensors"):
                try:
                    _models[k] = models.from_pretrained(local_model)
                except Exception as e:
                    raise RuntimeError(f"Failed to load local Pixal3D model component '{k}' from {local_model}") from e
            else:
